...
export const noticeTemplate = `@if(visible()){<div class="notice" [attr.role]="live()==='assertive'?'alert':live()==='polite'?'status':null" [attr.aria-live]="live()" aria-atomic="true" (mouseenter)="hover(true)" (mouseleave)="hover(false)" (focusin)="focus(true)" (focusout)="focus(false)">@if(showIcon()){<dl-icon [name]="resolvedIcon()" [size]="20"/>}<div class="copy">@if(heading()){<strong>{{heading()}}</strong>}<span>{{message()}}</span><ng-content/></div>@if(actionLabel()){<button class="action" type="button" (click)="act()">{{actionLabel()}}</button>}@if(dismissible()){<button class="close" type="button" [attr.aria-label]="dismissLabel()" (click)="dismiss()"><dl-icon name="x" [size]="16"/></button>}@if(showProgress()&&duration()>0){<span class="progress" aria-hidden="true" [style.width.%]="progress()"></span>}</div>}`;
...
@media(max-width:480px){
  ...
  .action{max-width:100%;white-space:normal;overflow-wrap:anywhere}
}
`;
