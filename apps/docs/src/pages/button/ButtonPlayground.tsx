  ...
  return (
    ...
    <div className="grid overflow-hidden rounded-3xl border border-white/10 bg-zinc-950 lg:grid-cols-[300px_1fr]">
      <form className="space-y-5 border-b border-white/10 p-6 lg:border-b-0 lg:border-r" onSubmit={(event) => event.preventDefault()}>
        ...
        <DocsToggle checked={disabled} label="Disabled" onChange={(value) => { setDisabled(value); if (value) setLoading(false); }} />
        <DocsToggle checked={loading} label="Loading" onChange={(value) => { setLoading(value); if (value) setDisabled(false); }} />
        <label className="control"><span>Loading label</span><input value={loadingLabel} onChange={(event) => { setLoadingLabel(event.target.value); setLoading(Boolean(event.target.value)); if (event.target.value) setDisabled(false); }} /></label>
    ...
    <section aria-live="polite" className="overflow-hidden rounded-2xl border border-white/10 bg-white/[.025]">
      <header className="flex flex-wrap items-start justify-between gap-4 border-b border-white/10 p-5"><div><h3 className="font-semibold text-white">Outputs and events</h3><p className="mt-1 text-sm text-zinc-400">Interact with the enabled button to inspect its native click event.</p></div><div className="text-right"><span className="block text-xs text-zinc-500">Click count</span><strong className="font-mono text-emerald-300">{clickCount}</strong></div></header>
      <div className="grid gap-px bg-white/10 sm:grid-cols-[12rem_minmax(0,1fr)]"><div className="bg-zinc-950 p-5"><span className="text-xs text-zinc-500">Latest event</span><strong className="mt-2 block font-mono text-sm text-white">{lastEvent}</strong></div><div className="bg-zinc-950 p-5">{events.length ? <ol className="space-y-2">{events.map((entry, index) => <li className="font-mono text-xs text-zinc-400" key={`${entry}-${index}`}>{entry}</li>)}</ol> : <p className="text-sm text-zinc-500">{disabled || loading ? "Events are suppressed while the button is disabled or loading." : "No events emitted yet. Click the preview button."}</p>}</div></div>
    </section>
    </div>
  );
}
