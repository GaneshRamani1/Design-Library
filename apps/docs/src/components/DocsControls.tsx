...
export function DocsToggle({
  ...
  useEffect(() => {
    void setCustomElementProperties(ref.current, {
      value: checked,
      label,
      showLabel: false,
    });
  ...
  useEffect(() => {
    ...
    const listener = (event: Event) =>
      onChange(Boolean((event as CustomEvent).detail));
  ...
  return (
    <div className="flex items-center justify-between gap-4 text-sm text-zinc-300">
      {showTextLabel && <span>{label}</span>}
      <arc-docs-toggle className={showTextLabel ? "" : "ml-auto"} ref={ref} />
    </div>
  );
}
...
export function DocsDropdown({
  ...
  useEffect(() => {
    const element = ref.current!;
    const listener = (event: Event) =>
      onChange(String((event as CustomEvent).detail));
    element.addEventListener("valueChange", listener);
    return () => element.removeEventListener("valueChange", listener);
  }, [onChange]);
...
export function DocsSegmented({
  ...
  }, [id, label, options, value, showLabel, orientation]);
  useEffect(() => {
    const element = ref.current!;
    const listener = (event: Event) =>
      onChange(String((event as CustomEvent).detail));
...
export function DocsSelectableList({
  id,
  label,
  options,
  value,
  ...
  }, [id, label, options, value, showLabel]);
  useEffect(() => {
    const element = ref.current!;
    const listener = (event: Event) =>
      onChange(String((event as CustomEvent).detail));
...
export function DocsTabs({
  ...
  useEffect(() => {
    void setCustomElementProperties(ref.current, { options, value });
  }, [value]);
  useEffect(() => {
    const element = ref.current!;
    const listener = (event: Event) =>
