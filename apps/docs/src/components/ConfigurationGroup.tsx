...
export function ConfigurationGroup({ component, examples }: { component: ComponentDoc; examples: ConfigurationExample[] }) {
  ...
  return <section id={anchor} className="scroll-mt-24 overflow-hidden rounded-3xl border border-white/10"><header className="flex flex-wrap items-center justify-between gap-4 bg-white/[.025] p-5"><div><code className="text-base text-emerald-300">{selected.property}</code><p className="mt-1 text-xs text-zinc-600">Change the value to update this example.</p></div>{booleanChoice ? <DocsToggle checked={enabled} label={selected.property} onChange={(nextValue) => { const next = values.findIndex((value) => value === nextValue); if (next >= 0) setSelectedIndex(next); }} /> : examples.length > 1 ? <DocsSegmented id={`${component.slug}-${selected.property}`} label={selected.property} options={examples.map((example, index) => ({ value: String(index), label: choiceLabel(example) }))} value={String(selectedIndex)} onChange={(value) => setSelectedIndex(Number(value))} /> : null}</header><ConfigurationRenderer component={component} example={selected} embedded /></section>;
}
