...
const navigation = [
  ...
  { label: "Foundations", items: [{ slug: "installation", title: "Installation", path: "/foundations/installation" }, { slug: "tokens", title: "Tokens", path: "/foundations/tokens" }] },
  ...rootOrder.map((label) => ({ label, items: (componentGroups[label] ?? []).sort((a, b) => a.title.localeCompare(b.title, undefined, { numeric: true })).map((item) => ({ ...item, path: `/components/${item.slug}` })) })),
];
    ...
    <div className="min-h-screen bg-black text-zinc-300">
      <header className="fixed inset-x-0 top-0 z-50 flex h-16 items-center border-b border-white/10 bg-black/80 px-4 backdrop-blur-xl md:px-6">
        ...
        <div className="ml-auto flex items-center gap-2">
          <button className="rounded-xl border border-white/10 px-3 py-2 text-sm text-zinc-300" onClick={() => setTheme(theme === "dark" ? "light" : "dark")}>{theme === "dark" ? "Light" : "Dark"}</button>
          <a className="rounded-xl bg-white px-3 py-2 text-sm font-semibold text-black" href="https://github.com/">GitHub ↗</a>
        </div>
      </header>
        ...
        <nav className="mt-8 space-y-7">
          {navigation.map((group) => (
            <section key={group.label}>
              <p className="mb-2 px-3 text-[11px] font-semibold uppercase tracking-[.18em] text-zinc-600">{group.label}</p>
              {group.label === "Foundations" && <p className="mb-1 mt-3 px-3 text-xs font-medium text-zinc-500">Getting started</p>}
              {group.items.map((item) => (
                <NavLink key={`${group.label}-${item.slug}`} to={item.path} end={item.path === "/"} className={({ isActive }) => `mt-1 block rounded-xl px-3 py-2.5 text-sm transition ${group.label === "Foundations" ? "ml-2" : ""} ${isActive ? "bg-white/10 font-medium text-white" : "text-zinc-400 hover:bg-white/5 hover:text-white"}`}>{item.title}</NavLink>
              ))}
            </section>
          ))}
        </nav>
