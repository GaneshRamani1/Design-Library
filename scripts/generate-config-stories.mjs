...
const meta: Meta<${name}> = { ...baseMeta, id: ${JSON.stringify(toId(overviewTitle))}, title: ${JSON.stringify(overviewTitle)}, tags: [], argTypes: { ...baseMeta.argTypes, ...${JSON.stringify(descriptions)} } };
export default meta;
export const Overview: StoryObj<${name}> = { ${hasDefault ? "...baseStory," : ""} parameters: { storyNote: "Start here: configure the component using Controls below. Browse individual settings next, then Variations, Events and Appearance.", controls: { expanded: true, sort: "requiredFirst" } } };
`,
  );
  records.unshift({
    prop: "overview",
